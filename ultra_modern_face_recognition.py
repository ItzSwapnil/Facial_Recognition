...
class UltraModernFaceRecognition:
    ...
    def add_known_face(self, image: np.ndarray, person_name: str, angle_type: str = "frontal") -> bool:
        ...
        self.console.print(
            f"   Embedding size: {face_encoding.embedding_size}, Detection score: {face_encoding.detection_score:.3f}, Angle: {angle_type}",
            style="cyan")
    ...
    def merge_persons(self, source_name: str, target_name: str):
        ...
        if merged_count > 0:
            self.save_known_faces()
            self.console.print(f"✅ Merged {merged_count} encoding(s) from '{source_name}' to '{target_name}'",
                               style="green")
            return True
        else:
    ...
    def add_known_face_enhanced(self, frame, person_name: str, angle_type: str = "frontal"):
        ...
        if encoding is not None:
            ...
            self.console.print(
                f"   Embedding size: {len(encoding)}, Detection score: {confidence:.3f}, Angle: {angle_type}",
                style="cyan")
        ...
        while True:
            ...
            if choice == '1':
                ...
                if name and name in person_stats:
                    confirm = input(
                        f"⚠️ Are you sure you want to delete all data for '{name}'? (y/N): ").strip().lower()
                    if confirm == 'y':
    ...
    while True:
        try:
            ...
            if choice == '1':
                ...
                if model_choice == '1':
                    # Quick 3D capture
                    angles = [("frontal", "Look straight"), ("left_profile", "Turn left"),
                              ("right_profile", "Turn right")]
                ...
                table.add_row("YuNet Face Detection",
                              "✅ Available" if hasattr(fr_system, 'face_detector') else "❌ Not Available", "2025 SOTA")
                table.add_row("SFace Recognition", "✅ Available" if fr_system.face_recognizer else "❌ Not Available",
                              "2025 SOTA")
                table.add_row("ONNX Runtime", "✅ Available" if ONNX_AVAILABLE else "❌ Not Available",
                              "GPU Optimized" if ONNX_AVAILABLE else "N/A")
